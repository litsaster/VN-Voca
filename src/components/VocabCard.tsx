...
export default function VocabCard({ item, learned, onToggleLearned }: Props) {
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-shadow no-underline"
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-end gap-2 mt-3" onClick={e => e.preventDefault()}>
          <button
            onClick={e => { e.preventDefault(); e.stopPropagation(); onToggleLearned(item.id) }}
